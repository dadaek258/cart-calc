...
struct ContentView: View {
    ...
    var body: some View {
        TabView(selection: $selectedTab) {
            MemoView(transferItemText: $transferItemText, selectedTab: $selectedTab)
                .tabItem {
    ...
    private struct MemoView: View {
        ...
        private struct MemoItem: Identifiable, Equatable {
            let id = UUID()
            var text: String
    ...
    private struct CalculatorView: View {
        ...
        private func recognizeText(from image: UIImage) {
            ...
            let request = VNRecognizeTextRequest { request, error in
                ...
                // 간단 파서: 품명과 가격 분리
                // 품명은 첫 숫자가 나오기 전까지, 가격은 첫 숫자부터 모두 붙여서 숫자만 추출
                var foundName = ""
                var foundPriceString = ""
                var foundPrice = 0
                
                // 공백 기준으로 탐색, 첫 숫자 포함 단어부터 가격으로 인식
